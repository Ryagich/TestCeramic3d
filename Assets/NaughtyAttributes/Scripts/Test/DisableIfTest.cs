...
namespace NaughtyAttributes.Test
{
    ...
    public class DisableIfNest1
    {
        ...
        [DisableIf(EConditionOperator.And, "Disable1", "Disable2")] [AllowNesting]
        // Because it's nested we need to explicitly allow nesting
        public int disableIfAll = 1;

        [DisableIf(EConditionOperator.Or, "Disable1", "Disable2")] [AllowNesting]
        // Because it's nested we need to explicitly allow nesting
        public int disableIfAny = 2;

        [DisableIf("Enum1", DisableIfEnum.Case1)] [AllowNesting]
        // Because it's nested we need to explicitly allow nesting
        public int disableIfEnum = 3;

        [DisableIf("Enum2", DisableIfEnumFlag.Flag0)] [AllowNesting]
        // Because it's nested we need to explicitly allow nesting
        public int disableIfEnumFlag;

        [DisableIf("Enum2", DisableIfEnumFlag.Flag0 | DisableIfEnumFlag.Flag1)] [AllowNesting]
        // Because it's nested we need to explicitly allow nesting
        public int disableIfEnumFlagMulti;
    ...
    public class DisableIfNest2
    {
        ...
        [DisableIf(EConditionOperator.And, "GetDisable1", "GetDisable2")] [MinMaxSlider(0.0f, 1.0f)]
        ...
        [DisableIf(EConditionOperator.Or, "GetDisable1", "GetDisable2")] [MinMaxSlider(0.0f, 1.0f)]
        // AllowNesting attribute is not needed, because the field is already marked with a custom naughty property drawer
        public Vector2 enableIfAny = new Vector2(0.25f, 0.75f);

        [DisableIf("GetEnum1", DisableIfEnum.Case2)] [MinMaxSlider(0.0f, 1.0f)]
        // AllowNesting attribute is not needed, because the field is already marked with a custom naughty property drawer
        public Vector2 enableIfEnum = new Vector2(0.25f, 0.75f);

        [DisableIf("GetEnum2", DisableIfEnumFlag.Flag0)] [MinMaxSlider(0.0f, 1.0f)]
        // AllowNesting attribute is not needed, because the field is already marked with a custom naughty property drawer
        public Vector2 disableIfEnumFlag;

        [DisableIf("GetEnum2", DisableIfEnumFlag.Flag0 | DisableIfEnumFlag.Flag1)] [MinMaxSlider(0.0f, 1.0f)]
        // AllowNesting attribute is not needed, because the field is already marked with a custom naughty property drawer
